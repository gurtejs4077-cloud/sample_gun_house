...
async function init() {
  ...
  document.getElementById('p-type').addEventListener('change', (e) => {
    ...
    document.getElementById('p-condition-group').style.display = (val === 'pre-owned') ? 'block' : 'none';
    
    // Auto-uncheck license for non-firearm types
    if (val === 'accessories' || val === 'co2') {
      const licCheck = document.getElementById('p-license');
      licCheck.checked = false;
      document.getElementById('license-type-group').style.display = 'none';
      
      // Also try to auto-select the right category if available
      const catSelect = document.getElementById('p-category');
      if (val === 'accessories') catSelect.value = 'accessories';
      if (val === 'co2') catSelect.value = 'co2-guns';
    }
  });
  ...
  document.getElementById('p-image-file').addEventListener('change', async (e) => {
    ...
    if (file) {
      ...
      formData.append('folder', 'products'); // Use products subfolder
...
async function seedMissingProducts() {
  if (!confirm('This will import missing products and refresh categories from local files. Proceed?')) return;
  
  let addedCount = 0;
  if (typeof PRODUCTS !== 'undefined') {
    PRODUCTS.forEach(p => {
      const exists = localProducts.some(lp => lp.id === p.id);
      if (!exists) {
  ...
  // Refresh Categories to ensure division mappings are correct
  if (typeof CATEGORIES !== 'undefined') {
    CATEGORIES.forEach(c => {
      const existing = localCategories.find(lc => lc.slug === c.slug);
      if (existing) {
        existing.division = c.division; // update division
  ...
  if (addedCount > 0) {
    renderTable();
    showAdminNotification(`✅ Imported ${addedCount} missing products and synced categories.`);
  } else {
    showAdminNotification('ℹ️ Categories synced. All local products are already in the dashboard.');
  }
}
...
function renderTable() {
  const mainTbody = document.getElementById('admin-product-table');
  const co2Tbody = document.getElementById('admin-co2-product-table');
  if (mainTbody) mainTbody.innerHTML = '';
  if (co2Tbody) co2Tbody.innerHTML = '';

  localProducts.forEach((p, index) => {
    ...
    if (p.type === 'co2') {
      if (co2Tbody) co2Tbody.innerHTML += row;
    } else {
      if (mainTbody) mainTbody.innerHTML += row;
    }
  });
}

function renderCategories() {
  const mainTbody = document.getElementById('admin-main-category-table');
  const co2Tbody = document.getElementById('admin-co2-category-table');
  if (mainTbody) mainTbody.innerHTML = '';
  if (co2Tbody) co2Tbody.innerHTML = '';

  localCategories.forEach((c, index) => {
    const row = `
    <tr>
    ...
    if (c.division === 'co2' || c.division === 'both' || !c.division) {
      if (co2Tbody) co2Tbody.innerHTML += row;
    }
    if (c.division === 'main' || c.division === 'both' || !c.division) {
      if (mainTbody) mainTbody.innerHTML += row;
    }
  });
}
...
function renderBrands() {
  const mainTbody = document.getElementById('admin-main-brand-table');
  const co2Tbody = document.getElementById('admin-co2-brand-table');
  if (mainTbody) mainTbody.innerHTML = '';
  if (co2Tbody) co2Tbody.innerHTML = '';
  ...
  localBrands.forEach((b, index) => {
    ...
    const div = b.division || 'both';
    if (div === 'co2' || div === 'both') {
      if (co2Tbody) co2Tbody.innerHTML += row;
    }
    if (div === 'main' || div === 'both') {
      if (mainTbody) mainTbody.innerHTML += row;
    }
  });
}

function addCategory(context = 'main') {
  const name = prompt('Category Name (e.g. Tactical Gear):');
  if (!name) return;
  
  const slug = name.toLowerCase().replace(/\s+/g, '-');
  localCategories.push({ slug, name, division: context });
  renderCategories();
  populateSelects();
}
...
function addBrand(context = 'main') {
  const name = prompt('Brand Name:');
  if (!name) return;
  const auth = confirm('Is this an authorized brand?');
  const slug = name.toLowerCase().replace(/\s+/g, '-');
  localBrands.push({ slug, name, authorized: auth, division: context });
  renderBrands();
  populateSelects();
}
...
function populateSelects(context = 'main') {
  const catSelect = document.getElementById('p-category');
  const typeSelect = document.getElementById('p-type');
  
  if (context === 'co2') {
    typeSelect.innerHTML = `
      <option value="co2">CO2 Gun</option>
      <option value="accessories">Accessories</option>
    `;
    catSelect.innerHTML = localCategories
      .filter(c => c.division === 'co2' || c.division === 'both' || !c.division)
      .map(c => `<option value="${c.slug}">${c.name}</option>`)
      .join('');
  } else {
    typeSelect.innerHTML = `
      <option value="new">New Firearms</option>
      <option value="pre-owned">Pre-owned</option>
      <option value="ammunition">Ammunition</option>
      <option value="accessories">Accessories</option>
    `;
    catSelect.innerHTML = localCategories
      .filter(c => c.division === 'main' || c.division === 'both' || !c.division)
      .map(c => `<option value="${c.slug}">${c.name}</option>`)
      .join('');
  }
...
function openAddModal(defaultType) {
  const context = (defaultType === 'co2') ? 'co2' : 'main';
  ...
  if (defaultType) {
    ...
    // Dispatch change event to trigger other handlers like hiding license checkbox
...
function openEditModal(index) {
  ...
  const context = (p.type === 'co2') ? 'co2' : 'main';
  populateSelects(context);
  ...
  if (typeof p.image === 'string' && !p.image.startsWith('SVG_')) {
    ...
    if (window.SVG_MAP && window.SVG_MAP[p.image]) {
      placeholder.innerHTML = window.SVG_MAP[p.image];
    }
...
function saveProduct() {
  ...
  if (imageType === 'upload') {
    ...
    if (category === 'accessories' || category === 'holsters' || category === 'optics' || category === 'cleaning') imageSource = 'SVG_HOLSTER';
  }
  ...
  try {
    ...
    const cards = enquiries.map(e => {
      try {
        ...
        const divisionBadge = e.division ? `<span style="background: ${e.division === 'CO2' ? '#38bdf8' : '#d4af37'}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 700; margin-right: 5px;">${e.division.toUpperCase()}</span>` : '';
        const typeBadge = e.type === 'cart-order' ? `<span style="background: #a855f7; color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 700;">CART</span>` : '';

        let itemsHtml = '';
        if (e.type === 'cart-order' && e.items) {
          itemsHtml = `<div style="margin: 0.5rem 0; font-size: 0.8rem; background: #f8fafc; padding: 0.5rem; border-radius: 4px;">
            ${e.items.map(item => `<div>• ${item.qty}x ${item.name} (${formatInr(item.price * 100)})</div>`).join('')}
            <div style="font-weight: 700; margin-top: 0.25rem;">Total: ₹${e.total?.toLocaleString('en-IN')}</div>
          </div>`;
        }
          ...
          <div class="enquiry-header" style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.5rem;">
            <div>
              <div style="display: flex; align-items: center; margin-bottom: 2px;">
                ${divisionBadge} ${typeBadge}
              </div>
              <h4 style="margin:0;">${customerName} <small style="font-weight:normal;color:var(--charcoal-500);">(${e.city || 'N/A'}, ${e.state || 'N/A'})</small></h4>
              <p style="margin:0.25rem 0;font-size:0.9rem;"><strong>${e.type === 'cart-order' ? 'Selection:' : 'Product:'}</strong> ${e.productName || 'Cart Order'}</p>
              ${itemsHtml}
            </div>
        ...
        console.error("Error rendering individual enquiry card:", err, e);
        return `<div style="border:1px solid #fecaca; padding:1rem; border-radius:8px; background:#fef2f2; color:#b91c1c;">Error rendering enquiry: ${e.id || 'Unknown ID'}</div>`;
      }
    });

    container.innerHTML = cards.join('');
  } catch (error) {
    console.error("Error in loadAndRenderEnquiries:", error);
    container.innerHTML = `<p style="color: #ef4444; text-align: center; padding: 1rem;">❌ Failed to load enquiries: ${error.message}</p>`;
  }
}
